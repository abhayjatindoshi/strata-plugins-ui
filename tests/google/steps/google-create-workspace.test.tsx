...
const pickedSpace = { id: 'drive', displayName: 'My Drive' };
...
function renderStep(service: CloudFileService) {
  ...
  const step = googleCreateWorkspaceStep({ service, mode: 'light' });
...
describe('googleCreateWorkspaceStep', () => {
  ...
  it('creates a private workspace in appDataFolder', async () => {
    ...
    expect(service.createFolder).toHaveBeenCalledWith(
      { id: 'appDataFolder', displayName: 'App data' },
      'My WS',
      null,
    );
    expect(onComplete).toHaveBeenCalledWith({
      name: 'My WS',
      space: { id: 'appDataFolder', displayName: 'App data' },
      folderId: 'created',
      shareable: false,
    });
  });
  ...
  it('auto-fills the name from the folder, then clears the folder when sharing is disabled', async () => {
    ...
    // disabling sharing clears the selected folder
    fireEvent.click(screen.getByText('Enable sharing with other users'));
  ...
  it('gates the explorer to Drive folders', () => {
    renderStep(makeService());
    ...
    expect(v.isSpaceVisible({ id: 'drive', displayName: '' })).toBe(true);
    expect(v.isSpaceVisible({ id: 'sharedWithMe', displayName: '' })).toBe(true);
    expect(v.isSpaceVisible({ id: 'other', displayName: '' })).toBe(false);
    expect(v.isSpaceEnabled({ id: 'drive', displayName: '' })).toBe(true);
    expect(v.isFileVisible(folder)).toBe(true);
    expect(v.isFileVisible(doc)).toBe(false);
    expect(v.isFileEnabled(folder)).toBe(true);
    expect(v.isFileEnabled(doc)).toBe(false);
    expect(v.folderCreationEnabled({ id: 'drive', displayName: '' }, null)).toBe(true);
  });
});
